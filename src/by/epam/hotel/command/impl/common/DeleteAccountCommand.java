...
public class DeleteAccountCommand implements ActionCommand {
	 ...
	 * page. If current user does not exist or current account can not be deleted,
	 * method will return back client or admin to previous page with according
	 * information. Otherwise method will delelte account of current user and send
	 * him by {@link by.epam.hotel.util.type.RouterType#REDIRECT REDIRECT} to page
	 * with successfull deletion information.
	 */
	@Override
			...
			try {
				if (CommonService.checkLogin(currentLogin, password)) {
					if (CommonService.deleteAccount(currentLogin)) {
						...
						request.setAttribute(AttributeConstant.ERROR_DELETE_ACCOUNT_MESSAGE, MessageManager
								.getProrerty(PropertyConstant.MESSAGE_DELETE_ACCOUNT_ERROR, sessionData.getLocale()));
						page = ConfigurationManager.getProperty(PropertyConstant.PAGE_DELETE_ACCOUNT);
						router.setType(RouterType.FORWARD);
					}
				} else {
					request.setAttribute(AttributeConstant.ERROR_CHECK_LOGIN_PASSWORD_MESSAGE, MessageManager
							.getProrerty(PropertyConstant.MESSAGE_CHECK_LOGIN_PASSWORD_ERROR, sessionData.getLocale()));
					page = ConfigurationManager.getProperty(PropertyConstant.PAGE_DELETE_ACCOUNT);
					router.setPage(page);
					router.setType(RouterType.FORWARD);
				}
