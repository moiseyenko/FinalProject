 ...
 * used to approve cancel of specified order by admin.
 * 
 * 
...
public class ApproveAdminOrderCancelCommand implements ActionCommand {
	 ...
	 * by {@link by.epam.hotel.util.type.RouterType FORWARD} to welcome page. If the
	 * order with specified parameters cannot be canceled, method will return admin
	 * by {@link by.epam.hotel.util.type.RouterType#FORWARD FORWARD} to previous
	 * page. Otherwise method will cancel specified order and return admin by
		...
		if (sessionData.getRole() == RoleType.ADMIN) {
			...
			try {
				if (AdminService.approveCancelOrder(accountId, returnedSum, orderId)) {
					...
					request.setAttribute(AttributeConstant.ERROR_ADMIN_ORDER_CANCEL_MESSAGE, MessageManager
							.getProrerty(PropertyConstant.MESSAGE_ADMIN_ORDER_CANCEL_ERROR, sessionData.getLocale()));
