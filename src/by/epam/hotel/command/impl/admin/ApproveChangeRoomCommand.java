 ...
 * used to approve modification of specified room.
 * 
 * 
...
public class ApproveChangeRoomCommand implements ActionCommand {
	 ...
	 * page. If parameters: capacity and price are invalid or if the room with
	 * specified parameters cannot be updated, method will return user by
	 * {@link by.epam.hotel.util.type.RouterType#FORWARD FORWARD} to previous page.
	 * Otherwise method will approve modification of specified room and return admin
	 * by {@link by.epam.hotel.util.type.RouterType#REDIRECT REDIRECT} to page with
	 * all rooms.
	 */
	@Override
				...
				try {
					...
					try {
						if (AdminService.approveChangeRoom(updatedRoom)) {
							...
							request.setAttribute(AttributeConstant.ERROR_CHANGE_ROOM_MESSAGE, MessageManager
									.getProrerty(PropertyConstant.MESSAGE_CHANGE_ROOM_ERROR, sessionData.getLocale()));
